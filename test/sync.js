describe('Checkit - sync', function() {
    ...
    describe('validators', function() {
        ...
        describe('range', function() {
            ...
            it('should pass for numbers in strings', function() {
                deepEqual(
                    ...
                    { stringInteger: testBlock.stringInteger }
                );
            });
        ...
        describe('emails', function() {
            ...
            it('fails with an invalid email', function() {
                return Checkit({ emailFail: ['email'] })
                    .run(testBlock)
                    .catch(function(err) {
                        equal(
                            err.get('emailFail').toString(),
                            'emailFail: The emailFail must be a valid email address'
                        );
                    });
            });
        });
    ...
    describe('custom validation objects', function() {
        ...
        it('allows for custom labels and messages', function() {
            return Checkit(vals)
                ...
                .then(null, function(err) {
                    equal(
                        err.get('first_name').message,
                        'The first name of this application must be at least 3 characters long'
                    );
                });
        });
        ...
        it('allows for custom params', function() {
            ...
            return Checkit(containsTest)
                .run({ arr: [0, 10, 20] })
                .then(function() {
                    return Checkit(_.extend(containsTest, { arr: 'contains:10' })).run({
                        arr: [0, 10, 20],
                    });
                })
                .then(null, function(err) {
    ...
    describe('custom validation rules', function() {
        ...
        it('should fail when the validation rule throws an error', function() {
            ...
            return Checkit(rulesTest)
                .run({ failedRuleTest: 'value' })
                .then(null, function(err) {
    ...
    describe('conditional items', function() {
        ...
        it('validates for items that pass the conditional', function() {
            return checkit
                .run({ email: 'joe@gmail.com', first_name: 'tim' })
                .then(function() {
    ...
    describe('nested items', function() {
        it('validates for nested items', function() {
            return Checkit({ 'info.email': ['required', 'email'] }).runSync({
                info: { email: 'joe@gmail.com' },
            });
        });
    });
});
