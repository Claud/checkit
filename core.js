...
module.exports = function(_, Promise) {
    ...
    Checkit.prototype.run = Checkit.prototype.validate = function(target, context, scenario = 'default') {
        return new Runner(this, target, context, scenario).run();
    };

    // Synchronously runs a validation block, returning an object of all fields
    // validated, or throwing a `Checkit.Error` object.
    Checkit.prototype.runSync = Checkit.prototype.validateSync = function(target, context, scenario = 'default') {
        try {
            return [null, new SyncRunner(this, target, context, scenario).run()];
        } catch (err) {
            return [err, null];
        }
    };
    ...
    Checkit.prototype.getMessage = function(item, key) {
        ...
        let label = item.label || this.labels[key] || language.labels[key] || this.labelTransform(key);
        let message =
            item.message || this.messages[item.rule] || language.messages[item.rule] || language.messages.fallback;
        let labelRegex = /\{\{label\}\}/g;
        message = message.replace(labelRegex, label);
        if (item.params) {
            for (let i = 0, l = item.params.length; i < l; i++) {
    ...
    function checkSync(validations, input, key) {
        let arr = new Checkit(validations).runSync(input);
    ...
    function Runner(checkit, target, context, scenario) {
        this.errors = {};
    ...
    Runner.prototype.run = function() {
        ...
        let validationHash = _.clone(this.checkit.validations);
        let errors = {};
        ...
        return Promise.all(pendingConditionals)
            ...
            .then(function() {
                if (!_.isEmpty(errors)) {
                    let err = new CheckitError(_.keys(errors).length + ' invalid values');
                    err.errors = errors;
                    throw err;
                }
    ...
    function processItem(runner, currentValidation, key, context) {
        let value = getVal(runner.target, key);
        let rule = currentValidation.rule;
        let params = [value].concat(currentValidation.params).concat(context);
    ...
    SyncRunner.prototype.run = function() {
        ...
        _.each(this.conditional, function(conditional) {
            let result = checkConditional(runner, conditional);
    ...
    _.extend(Validator.prototype, {
        ...
        between: function(val, min, max) {
            return this.greaterThan(val, min) && this.lessThan(val, max);
        },
        ...
        range: function(val, min, max) {
            return this.greaterThanEqualTo(val, min) && this.lessThanEqualTo(val, max);
        },
    ...
    function checkInt(val) {
        if (!val.match(Regex.integer)) throw new Error('The validator argument must be a valid integer');
    }

    function checkNumber(val) {
        if (!numeric(val)) throw new Error('The validator argument must be a valid number');
    }
    ...
    _.extend(FieldError.prototype, {
        ...
        toString: function(flat) {
            let errors = flat ? [this.errors[0]] : this.errors;
            return this.key + ': ' + _.map(errors, 'message').join(', ');
        },
    ...
    let objMethods = ['keys', 'values', 'toPairs', 'invert', 'pick', 'omit'];
    let arrMethods = ['head', 'initial', 'tail', 'last'];
    let shareMethods = [
        'forEach',
        'each',
        'map',
    ...
    _.each(shareMethods.concat(objMethods), function(method) {
        CheckitError.prototype[method] = function() {
            return _[method](...[this.errors].concat(_.toArray(arguments)));
        };
    });
    ...
    function prepValidations(validations) {
        ...
        for (let key in validations) {
            ...
            for (let i = 0, l = validation.length; i < l; i++) {
    ...
    function assembleValidation(validation) {
        ...
        if (_.isString(validation.rule)) {
            let splitRule = validation.rule.split(':');
